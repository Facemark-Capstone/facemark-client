...
namespace mobile.Services
{
    ...
    {
        HttpClient _client;
        IOptionsSnapshot<CognitiveServiceOptions> mOptions { get; }
        ...
        public FaceRecognitionService(IOptionsSnapshot<CognitiveServiceOptions> options)
        {
            mOptions = options;
            _client = new HttpClient();
            _client.DefaultRequestHeaders.Add("ocp-apim-subscription-key", mOptions.Value.ApiKey);
        }
        ...
        {
            var requestUrl = $"{mOptions.Value.FaceEndpoint}/detect?returnFaceId={returnFaceId}&returnFaceLandmarks={returnFaceLandmarks}&returnFaceAttributes={GetAttributeString(returnFaceAttributes)}";
            return await SendRequestAsync<Stream, Face[]>(HttpMethod.Post, requestUrl, imageStream);
        }

        async Task<TResponse> SendRequestAsync<TRequest, TResponse>(HttpMethod httpMethod, string requestUrl, TRequest requestBody)
        {
            var request = new HttpRequestMessage(httpMethod, mOptions.Value.FaceEndpoint);
